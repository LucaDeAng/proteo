...
interface ProteoChatInputProps {
  onSendMessage?: (
    message: string,
...
const MARINE_MODELS: ModelOption[] = [
  {
    id: "proteo-marine-gpt4",
    name: "Proteo Marine GPT-4",
    description: "Specialized for marine data analysis",
    badge: "Marine AI",
  },
  {
    id: "claude-sonnet-4",
    name: "Claude Sonnet 4",
    description: "General purpose with marine context",
  },
  {
    id: "gpt-4o-mini",
    name: "GPT-4o Mini",
    description: "Fast marine responses",
  },
];

const MARINE_QUICK_ACTIONS: QuickAction[] = [
  {
    id: "temp",
    label: "Temperatura Marina",
    labelEn: "Sea Temperature",
    icon: <Thermometer className="h-4 w-4" />,
    prompt: "Qual è la temperatura attuale del mare nelle diverse zone del Mediterraneo?",
  ...
  },
  {
    ...
    prompt: "Mostrami i livelli di clorofilla attuali nel Mediterraneo e il loro significato",
    category: "biology"
  },
  {
    id: "waves",
  ...
  {
    id: "nav-data",
    label: "Come accedere ai dati",
    labelEn: "How to access data",
  ...
  },
  {
    ...
    prompt: "Analizza la qualità delle acque costiere italiane e i parametri chimico-fisici",
    category: "quality"
  },
  {
    id: "nav-publications",
    label: "Trova pubblicazioni",
    labelEn: "Find publications",
...
const QuickActionsBar: React.FC<{
  ...
  return (
    <div className="p-3 border-b border-ocean-200 bg-gradient-to-r from-ocean-50 to-cyan-50">
      <div className="flex flex-wrap gap-2">
        {actions.map((action) => (
          <Button
            ...
            className="h-8 px-3 text-xs border-ocean-200 hover:border-ocean-400 hover:bg-ocean-50 text-ocean-700 hover:text-ocean-800"
          >
            {action.icon}
            <span className="ml-1.5 hidden sm:inline">{action.label}</span>
            <span className="ml-1.5 sm:hidden">{action.labelEn}</span>
          </Button>
        ))}
      </div>
    </div>
...
const FilePreviewCard: React.FC<{
  ...
  return (
    <div
      className={cn(
        "relative group bg-white border border-ocean-200 rounded-lg shadow-md flex-shrink-0 overflow-hidden",
        "w-[125px] h-[125px]",
        isImage ? "p-0" : "p-3"
      )}
    >
          ...
          <div className="flex-1 min-w-0 overflow-hidden">
            <div className="flex items-center gap-1.5 mb-1">
              ...
              {file.uploadStatus === "uploading" && (
                <Loader2 className="h-3.5 w-3.5 animate-spin text-ocean-500" />
              )}
            ...
            <div className="absolute bottom-2 left-2">
              <span className="text-xs bg-ocean-100 text-ocean-700 px-2 py-1 rounded border">
      ...
      <Button
        size="icon"
        variant="outline"
        className="absolute top-1 right-1 h-6 w-6 p-0 opacity-0 group-hover:opacity-100 bg-white/90"
...
const TextualFilePreviewCard: React.FC<{
  ...
  return (
    <div className="bg-white border border-ocean-200 relative rounded-lg p-3 w-[125px] h-[125px] shadow-md flex-shrink-0 overflow-hidden">
      <div className="text-[8px] text-gray-700 whitespace-pre-wrap break-words max-h-16 overflow-y-auto">
        {file.textContent ? (
          <>
            {previewText}
            {needsTruncation && "..."}
...
const PastedContentCard: React.FC<{
  ...
  return (
    <div className="bg-white border border-ocean-200 relative rounded-lg p-3 w-[125px] h-[125px] shadow-md flex-shrink-0 overflow-hidden group">
      <div className="text-[8px] text-gray-700 whitespace-pre-wrap break-words max-h-16 overflow-y-auto">
        {previewText}
        {needsTruncation && "..."}
      </div>
      ...
      <div className="absolute bottom-2 left-2">
        <span className="text-xs bg-cyan-100 text-cyan-700 px-2 py-1 rounded border">
          PASTED
        </span>
      </div>
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <Button
        variant="ghost"
        size="sm"
        className="h-9 px-2.5 text-sm font-medium text-ocean-700 hover:text-ocean-800 hover:bg-ocean-100"
        onClick={() => setIsOpen(!isOpen)}
      >
        <Shell className="h-4 w-4 mr-1.5" />
      ...
      {isOpen && (
        <div className="absolute bottom-full right-0 mb-2 w-72 bg-white border border-ocean-200 rounded-lg shadow-xl z-20 p-2">
          {models.map((model) => (
            <button
              key={model.id}
              className={cn(
                "w-full text-left p-2.5 rounded-md hover:bg-ocean-50 transition-colors flex items-center justify-between",
                model.id === selectedModel && "bg-ocean-100"
              )}
              onClick={() => {
            ...
            >
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {model.badge && (
                    <span className="px-1.5 py-0.5 text-xs bg-ocean-500/20 text-ocean-700 rounded">
                      {model.badge}
                    </span>
                  )}
                </div>
...
// Main ProteoChatInput Component
const ProteoChatInput: React.FC<ProteoChatInputProps> = ({
  onSendMessage,
  disabled = false,
  placeholder = "Chiedimi informazioni sui dati marini ISPRA...",
  maxFiles = MAX_FILES,
  ...
  return (
    ...
    >
      {isDragging && (
        <div className="absolute inset-0 z-50 bg-ocean-100/90 border-2 border-dashed border-ocean-400 rounded-xl flex flex-col items-center justify-center pointer-events-none">
          <p className="text-sm text-ocean-700 flex items-center gap-2">
            <ImageIcon className="size-4 opacity-50" />
            Trascina i file qui per aggiungerli alla chat
          </p>
        </div>
      )}

      <div className="bg-white border-2 border-ocean-200 rounded-xl shadow-lg overflow-hidden">
        {/* Quick Actions */}
        {showQuickActions && (
          <QuickActionsBar
            actions={MARINE_QUICK_ACTIONS}
        ...
        <div className="flex flex-col min-h-[120px]">
          <textarea
            ...
            disabled={disabled}
            className="flex-1 min-h-[80px] w-full p-4 focus:outline-none max-h-[120px] resize-none border-0 bg-transparent text-gray-800 shadow-none focus-visible:ring-0 placeholder:text-gray-500 text-sm sm:text-base"
            rows={1}
          />
          
          {/* Controls */}
          <div className="flex items-center gap-2 justify-between w-full px-4 pb-3">
            <div className="flex items-center gap-2">
              <Button
                size="icon"
                variant="ghost"
                className="h-9 w-9 p-0 text-ocean-600 hover:text-ocean-700 hover:bg-ocean-100 flex-shrink-0"
        ...
        {(files.length > 0 || pastedContent.length > 0) && (
          <div className="overflow-x-auto border-t border-ocean-200 p-3 bg-ocean-50/50">
